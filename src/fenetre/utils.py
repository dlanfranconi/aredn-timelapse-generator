...
class GoProRequest:
    def __init__(
        self, scheme, ip_address, timeout=20, iface=None, root_ca_filepath=None
    ):
        self.scheme = scheme
    ...
    def log_request(self, what, content=None):
        if logging.getLogger().getEffectiveLevel() > logging.DEBUG:
            c = content
            try:
                content_length = len(c) if c is not None else 0
            except TypeError:
                content_length = 0
            if content_length > 512:
                c = "length: " + str(content_length)
        ...
        for attempt in range(max_retries):
            ...
            if self.iface:
                try:
                    ...
                    self.log_request(
                        f"GoProRequest: {url_path} {self.iface} exception in setting session {e}"
                    )
                    return r
            ...
            if self.root_ca_filepath is not None:
                request_kwargs["verify"] = self.root_ca_filepath
            r = session.get(
                f"{self.scheme}://{self.ip_address}{url_path}", **request_kwargs
            )
            self.log_request(
                f"GoProRequest.get() {url_path} {self.iface} Response: {r}", r.content
            )

            if (
                r.status_code == 500
            ):  # for some reason my gopro hero 12 sometimes returns 500 that are actualy 200
                r.status_code = 200
